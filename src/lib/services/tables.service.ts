 ...
 */
async function request<T>(endpoint: string, options?: RequestInit, baseUrl = BASE_URL): Promise<T> {
...
export const TablesService = {
  ...
  getAll(): Promise<Table[]> {
    return request<Table[]>("/api/tables", undefined, "");
  },
  ...
  /** POST /api/tables */
  create(payload: { placeId: number; name: string; status: "available" | "occupied" }): Promise<Table> {
